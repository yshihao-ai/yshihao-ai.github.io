...
export default function PublicationsList({ config, publications, embedded = false }: PublicationsListProps) {
    ...
    const groupedPublications = useMemo(() => {
        return [
            {
                key: 'first-author',
                title: messages.publications.firstAuthor,
                items: filteredPublications.filter(pub => pub.publicationGroup === 'first-author'),
            },
            {
                key: 'participating',
                title: messages.publications.participating,
                items: filteredPublications.filter(pub => pub.publicationGroup === 'participating'),
            },
                    ...
                    groupedPublications.map((group) => (
                        <section key={group.key} className="space-y-4">
                            ...
                            <div className="space-y-6">
                                {group.items.map((pub, index) => renderPublicationCard(pub, index))}
                            </div>
                        </section>
                    ))
                )}
